...
export default function MotionSystem() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    // Deferred until after load: this setup is ~100ms of main-thread work. Running it
    // in the frame straight after first paint held LCP back by ~0.9s — the hero was
    // painted but the browser could not present that frame until this finished. The
    // hero's own entrance is pure CSS, so nothing above the fold waits on GSAP.
    const startMotion = () => {
      if (cancelled) return;
      mm.add(
        {
        ...
        (self) => {
          ...
          /**
           * Builds a section's animations only once it is within ~1.5 viewports.
           *
           * Splitting every heading and creating every ScrollTrigger during hydration
           * cost ~1s of Style & Layout and pushed Total Blocking Time to 290ms. Arming
           * per section keeps that work off the critical path and splits it into tasks
           * far too small to block. The animations themselves are unchanged — each one
           * is still armed well before its section can scroll into view.
           */
          const armSection = (selector: string, setup: () => void) => {
            ...
            const observer = new IntersectionObserver(
              (entries) => {
                ...
                observer.disconnect();
                // No ScrollTrigger.refresh() here: each new trigger measures itself on
                // creation, and a global refresh re-measures every existing trigger —
                // eight of those during load put ~60ms back onto blocking time.
                setup();
              },
          ...
          if (firstLoad) {
            ...
            intro
              ...
              .from(".nav > .button", { opacity: 0, yPercent: -40, duration: 0.6 }, 0.28)
              .from(".nav-burger", { opacity: 0, yPercent: -40, duration: 0.6 }, 0.32)
              .from(".hero-actions > *", { opacity: 0, yPercent: 45, stagger: 0.1, duration: 0.8 }, 0.62)
              // Hero graphic: scale + soft slide, then the dashboard layers on top of it.
              .from(".hero-visual", { opacity: 0, xPercent: 6, scale: 0.96, duration: 1.25 }, 0.34)
              .from(".dashboard", { opacity: 0, y: 26, scale: 0.97, duration: 1 }, 0.6)
              .from(".system-badge", { opacity: 0, duration: 0.6 }, 0.8)
              .from(".dash-head, .dash-kpis > div", { opacity: 0, y: 12, stagger: 0.07, duration: 0.6 }, 0.78)
              .from(
                ".bars i",
            ...
            q(".dash-kpis strong").forEach((value, index) => countUp(value, intro, 0.85 + index * 0.06));
          }

          // The hero <h1> is deliberately NOT animated here. It is the LCP element,
          // and anything GSAP does to it can only start after this bundle downloads,
          // parses and hydrates — which measured 1.6s of pure LCP delay. Its reveal
          // lives in globals.css (@keyframes hero-line) and starts at first paint.
      ...
      // Initial states are in place — drop the pre-hydration paint guard.
    ...
    // Two frames after load: one to present the painted hero, one for safety.
    const onLoad = () => requestAnimationFrame(() => requestAnimationFrame(startMotion));
